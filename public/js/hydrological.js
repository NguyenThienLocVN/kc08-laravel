const loadHydrological = (id) => {
    ...
    success:function(data){
      ...
      $("#overlay").hide();
    //  Show station name
    $('#hydrologicalModalLabel').html("TỔNG HỢP SỐ LIỆU THỦY VĂN TRẠM "+data.station[0].Station_Name);
    $('#hydrological-station-value').html(data.station[0].Station_Name);
    $('#hydrological-longitude-value').html(data.station[0].Longitude);
    $('#hydrological-latitude-value').html(data.station[0].Latitude);
    $('#hydrological-river-value').html(data.station[0].Basin_Name);
    
    const arrayDate = [];
    // Get array of all date
     data.hydrological.forEach(function(e){
    ...
    document.getElementById('hydrological-start-picker').value = fromDate;
    document.getElementById('hydrological-end-picker').value = toDate;

    if(data.hydrological.length == 0)
    {
      $('#hydrological-start-picker').val("")
      $('#hydrological-end-picker').val("")
      $('#hydrological-monitoring-time').html("");
      alert("Dữ liệu trạm đang được cập nhật. Vui lòng thử lại sau.")
    }
    else
    {
      $('#hydrological-monitoring-time').html($('#hydrological-start-picker').val()+' - '+$('#hydrological-end-picker').val());
    }
    ...
    const filterArray = data.hydrological.slice(indexOfFrom, indexOfTo);
    ...
    //  Max do duc trong thang
     ...
     // Do duc trung binh cua thang
      ...
      function getMonthData(year){
        ...
        const filterArray = data.hydrological.slice(indexOfFirstDay, indexOfLastDay);
        ...
        var startYear = document.getElementById('hydrological-start-picker').value.slice(-4);
        if(startYear % 4 == 0){
     ...
     function averageTurbidityOfYear(year){
      ...
      else
      {
        ...
        $('#hydrological-average-value').html(avg.toFixed(1));
    ...
    function maxTurbidityOfYear(year){
      ...
      else
      {
        ...
        if(maxValue == 0)
        {
          $('#hydrological-max-value').html("0");
          $('#hydrological-max-date').html("0");
        }
        else
        {
          $('#hydrological-max-value').html(maxValue);
          $('#hydrological-max-date').html(dateMaxOfYear);
        }
        ...
        for(var i = 0; i <= 11; i++)
        {
          $(".hydrological-date-max-appear").find('.date').eq(i).html(indexOfDateInMonth(monthData['months'][i], parseFloat(maxMonth[i])))
        }
    ...
    //  Do duc nho nhat nam
     function minTurbidityOfYear(year){
      ...
      else
      {
        ...
        $('#hydrological-min-value').html(minValue);
        $('#hydrological-min-date').html(dateMinOfYear);

        // Get min date of each month
        for(var i = 0; i <= 11; i++)
        {
          $(".hydrological-date-min-appear").find('.date').eq(i).html(indexOfDateInMonth(monthData['months'][i], parseFloat(minMonth[i])))
        }
    ...
    function wetSeasonTurbidity(year){
      ...
      else
      {
        ...
        $('#hydrological-wet-season-min-value').html(minValue);
        ...
        $('#hydrological-wet-season-max-value').html(maxValue);

        if(data.hydrological.length == 0)
        {
          $('#hydrological-wet-season-max-date').html("0");
          $('#hydrological-wet-season-min-date').html("0");
        }
        else
        {
          $('#hydrological-wet-season-max-date').html(dateMaxOfWetSeason);
          $('#hydrological-wet-season-min-date').html(dateMinOfWetSeason);
        }
    }
    ...
    function drySeasonTurbidity(year){
      ...
      else
      {
        ...
        $('#hydrological-dry-season-min-value').html(minValue);
        ...
        $('#hydrological-dry-season-max-value').html(maxValue);
        
        if(data.hydrological.length == 0)
        {
          $('#hydrological-dry-season-min-date').html("0");
          $('#hydrological-dry-season-max-date').html("0");
        }
        else
        {
          $('#hydrological-dry-season-min-date').html(dateMinOfDrySeason);
          $('#hydrological-dry-season-max-date').html(dateMaxOfDrySeason);
        }
    }

    var startYear = document.getElementById('hydrological-start-picker').value.slice(-4);
    var endYear = document.getElementById('hydrological-end-picker').value.slice(-4);
    ...
    // Draw line chart 
    muddySandChart = Highcharts.chart('hydrological-container', {
      title: {
        text: 'BIỂU ĐỒ THỦY VĂN NGÀY'
      },
      yAxis: {
          title: {
              text: '(mm)'
    ...
    $('#hydrological-avg-chart').highcharts({
      chart: {
        type: 'column'
      },
      title: {
        text: 'BIỂU ĐỒ THỦY VĂN TRUNG BÌNH THÁNG / NĂM ' + startYear
      },
    xAxis: {
        categories: [
            'I',
    ...
    maxChart = Highcharts.chart('hydrological-max-chart', {
      chart: {
        type: 'column'
    },
    title: {
      text: 'BIỂU ĐỒ THỦY VĂN LỚN NHẤT THÁNG NĂM ' + startYear
    },
    xAxis: {
        categories: [
          'I',
    ...
    $("#btn-show-download").click(function(){
      ...
      $('#btn-export-png').click(function () {
        var chart = $('#hydrological-container').highcharts();
    ...
    document.getElementById('search-rain-btn').addEventListener('click', function(){
      var inputFrom = document.getElementById('hydrological-start-picker').value;
      var inputTo = document.getElementById('hydrological-end-picker').value;
      ...
      // Filter data
      var filterArrayByInput = data.hydrological.slice(indexOfInputFrom, indexOfInputTo);
      ...
      var turbidityChart = $('#hydrological-container').highcharts();
      ...
      if(startYear === endYear)
      {
        ...
        // Update max data
        var maxChart = $('#hydrological-max-chart').highcharts();
        ...
        // Update avg data
        var avgChart = $('#hydrological-avg-chart').highcharts();
...
function toPdf(){
  var val = htmlToPdfmake(document.getElementById('hydrological-info').outerHTML);
  var dd = {
    content:val,
    styles:{
      'font-weight-bold':{
